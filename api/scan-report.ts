...
function getSimulatedLabReport(hint?: string) {
  const isLipidOrKidney =
    hint &&
    (hint.toLowerCase().includes('lipid') ||
      hint.toLowerCase().includes('kidney') ||
      hint.toLowerCase().includes('creatinine'));
  const isDiabetic =
    hint &&
    (hint.toLowerCase().includes('sugar') ||
      hint.toLowerCase().includes('glucose') ||
      hint.toLowerCase().includes('diabetes'));
      ...
      return res.json({
        ...
        data: {
          ...
          diagnosticCenterName:
            parsed.diagnosticCenterName || 'District Pathology Laboratory',
